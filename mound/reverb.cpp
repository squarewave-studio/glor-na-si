...
int Reverb::Init(float sr)
{
    ...
    prv_lpfreq_    = 0.0;
    init_done_     = 1;
    // The lines follow one another in samples. A rate that needs more
    // than the buffer holds is refused before anything is touched.
    size_t total = 0;
    for(int i = 0; i < 8; i++)
        total += (size_t)DelayLineMaxSamples(sr, 1, i);
    if(total > kReverbMaxSize)
        return 1;
    size_t at = 0;
    for(int i = 0; i < 8; i++)
    {
        delay_lines_[i].buf = (aux_) + at;
        InitDelayLine(&delay_lines_[i], i);
        at += (size_t)DelayLineMaxSamples(sr, 1, i);
    }
    return 0;
}
...
void Reverb::NextRandomLineseg(ReverbDl *lp, int n)
{
...
int Reverb::InitDelayLine(ReverbDl *lp, int n)
{
    float read_pos;
    /* int     i; */
...
int Reverb::Process(const float &in1,
                      const float &in2,
                      float *      out1,
                      float *      out2)
{
